...
export default async function AdminCourseCategoriesPage({
  ...
  const t = await getTranslations({ locale, namespace: "admin.courseCategoriesPage" });
  ...
  async function createCategory(formData: FormData) {
    ...
    revalidatePath(`/${locale}/admin/course-categories`);
  }
              ...
              <div key={cat.id} className="flex items-center justify-between p-4 rounded-xl bg-surface border border-border">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-muted">{cat._count.courses} {t("coursesCount")}</p>
                  </div>
                </div>
